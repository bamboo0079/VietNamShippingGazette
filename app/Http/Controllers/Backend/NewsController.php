...
class NewsController extends Controller
{
    ...
    public function products(Request $request)
    {
        ...
        $data['books'] = $book->orderBy('id', 'DESC')->paginate($this->limit);
        return view('backend.news.products', $data);
    }

    public function add(Request $request, $book_id = 0)
    {
        $data = [];
        $data['categories'] = Category::whereNotIn('id', [2,3,4,5,17])->orderBy('order', 'ASC')->get();
        $data['productcategories'] = ProductCategory::orderBy('order', 'ASC')->get();
        return view('backend.news.edit', $data);
    }

    public function addProduct(Request $request, $book_id = 0)
    {
        $data = [];
        $data['categories'] = Category::orderBy('order', 'ASC')->get();
        $data['productcategories'] = ProductCategory::orderBy('order', 'ASC')->get();
        return view('backend.news.edit_product', $data);
    }
    ...
    public function detail(Request $request, $book_id = 0)
    {
        ...
        $data['book'] = $book;
        return view('backend.news.detail', $data);
    }

    public function detailProduct(Request $request, $book_id = 0)
    {
        $data = [];
        $data['categories'] = Category::whereNotIn('id', [2,3,4,5,17])->orderBy('order', 'ASC')->get();
        $data['productcategories'] = ProductCategory::orderBy('order', 'ASC')->get();
        ...
        $data['book'] = $book;
        return view('backend.news.detail_product', $data);
    }
    ...
    public function process(Request $request, $book_id = 0)
    {
        ...
        return redirect()->route('admin.news')->with('status', 'Lưu thông tin thành công');
    }

    public function processProduct(Request $request, $book_id = 0)
    {
        ...
        return redirect()->route('admin.new.product')->with('status', 'Lưu thông tin thành công');
    }
